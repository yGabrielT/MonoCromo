...
namespace SingularityGroup.HotReload.Editor {
    ...
    internal class HotReloadRunTab : HotReloadTabBase {
        ...
        internal void RenderPromoCodes() {
            _showPromoCode = EditorGUILayout.Foldout(_showPromoCode, "Promo Codes", true, HotReloadWindowStyles.FoldoutStyle);
            if (!_showPromoCode) {
                return;
            }
            if (promoCodeActivatedThisSession) {
                HotReloadGUIHelper.HelpBox($"Your promo code has been successfully activated. Free trial has been extended by 2 months.", MessageType.Info, 11);
            } else {
